...
La variable acc_now_delinq se puede transformar en 1 si tiene al menos una cuenta en mora, 0 sino tiene cunetas en mora.


```{python}
df["acc_now_delinq"]=np.where(df["acc_now_delinq"]>0, 1,0)
df["acc_now_delinq"]=df["acc_now_delinq"].astype("str")
...
Se realiza pruebas $\chi^2$ para las variables categoricas en contraste con la variable __good_status__ y se observa 6 variables con un p-valor pequeño, esto significa que estas variables pueden influir en el incumplimiento de las finanzas. Se excluye __pymnt_plan__ debido a un p-valor > 0.05.
